...
def MyShoppingCarts(request):
    try:
        ...
        return render(request, "MyCart1.html", {'data': CART_CONTAINER.values(), 'totalamount': total,
                                               'totalproducts': len(CART_CONTAINER.keys()), 'totalprice': totalprice,
                                               'totalsavings': totalsavings})

    except Exception as err:
        print("ERRORRRRR:", err)
        return render(request, "MyCart1.html", {'data': {}})
...
def InsertUser(request):
    ...
    try:
      DB, CMD = Pool.ConnectionPooling()
      Q = "insert into users values('{0}','{1}','{2}','{3}','{4}')".format(mobileno,emailid,firstname,lastname,password)
      CMD.execute(Q)
      DB.commit()
      DB.close()
...
def  IUserAddress(request):
    ...
    try:
      DB, CMD = Pool.ConnectionPooling()
      Q = "insert into users_address(mobileno, emailid, address1) values('{0}','{1}','{2}')".format(mobileno,emailid,addressone)
      print(Q)
      CMD.execute(Q)
      DB.commit()
      DB.close()
